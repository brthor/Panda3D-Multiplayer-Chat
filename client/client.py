...
class Client(DirectObject):
	def __init__(self,p,i):
		...
		self.Connection = self.cManager.openTCPClientConnection(self.ip,self.port,5000) #Create the connection
		if self.Connection:
...
class Terrain(GeoMipTerrain):
	def __init__(self):
		...
		self.terrain.setColorMap(Filename("terrain.bmp"))
...
class chatRegulator(DirectObject):
	def __init__(self,clientClass,keysClass):
		...
		#Create GUI
		#self.frame = 
		self.chatInput = DirectEntry(initialText = "Click me to chat!!!",cursorKeys = 1,numLines = 1,command = self.send,focusInCommand=self.clearText, focusOutCommand = self.resetText, focus = 0, width = 20)
		self.chatInput.setPos(-1.31667,0,-0.97)
		self.chatInput.setScale(0.0498732)
		self.messages = []
		self.txt = []
		for k in range(14):
		...
		self.accept("t",self.chatInput.setFocus)
		self.calls = 0
	def clearText(self):
		self.chatInput.enterText('')
		self.keys.isTyping = True
	def resetText(self):
		self.chatInput.enterText('Click me to chat!')
	...
	def setText(self,text):
		...
		for k in self.messageList:
			self.text(k,(-1.3, -.8+.06*self.index),self.index)
			self.index += 1
